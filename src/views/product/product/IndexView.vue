<template>
  <v-layout class="w-100 pa-2 d-flex flex-column py-10">
    ...
    <v-table class="w-100 pa-3">
      ...
      <tbody>
        <tr v-for="item in baseData.products?.list" :key="item.uuid">
          <td class="text-center">{{ item.uuid }}</td>
          <td class="text-center">{{ item.name }}</td>
          <td class="text-center">{{ item.new_item }}</td>
    ...
    <v-pagination :length="pagination"></v-pagination>
  </v-layout>
</template>
...
const baseData = computed(() => baseStore.base)
const pagination = computed(() =>
  baseData.value.products ? baseData.value.products.list.length / 20 : 1
)
